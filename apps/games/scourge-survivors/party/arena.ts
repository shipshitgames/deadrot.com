...
export default class Arena implements Party.Server {
  ...
  onMessage(raw: string, sender: Party.Connection) {
    ...
    if (m.t === "join") {
      ...
      p.joined = true;
      if (!wasJoined) {
        // Two sockets may connect before either sends `join`. The onConnect
        // welcome therefore cannot be the only roster snapshot: catch the new
        // joiner up with peers that became visible while it was half-connected.
        for (const peer of this.players.values()) {
          if (peer.id !== p.id && peer.joined) {
            sender.send(JSON.stringify({ t: "join", player: this.publicPlayer(peer) }));
          }
        }
        this.broadcastToJoined({ t: "join", player: this.publicPlayer(p) }, [sender.id]);
      }
