...
import { SHOP_BY_ID, SURVIVOR_RUN_GOAL_TIME, shopCost, runGold, xpForLevel, type ShopId, type SurvivorClassId } from './game/data/survivors'
import { MAGAZINE_SIZE, PLAYER_MAX_HEALTH, START_RESERVE, STARTING_WEAPON, TOTAL_WAVES, WEAPONS, type PickupKind, type WeaponId } from './game/constants'
...
export default function App() {
  ...
  const initialSandbox = useMemo(() => sandboxAvailable && new URLSearchParams(window.location.search).get('sandbox') === '1', [sandboxAvailable])
  ...
  useEffect(() => {
    ...
    if (import.meta.env.DEV) {
      ;(window as unknown as { __fpsGame?: Game; __fpsAudio?: typeof audio; __hudSnapshot?: () => HUDState }).__fpsGame = game
  ...
  const handleStartSurvivors = useCallback((classId?: SurvivorClassId) => {
    audio.unlock()
  ...
  const handleStartSandbox = useCallback((mapId?: string) => {
