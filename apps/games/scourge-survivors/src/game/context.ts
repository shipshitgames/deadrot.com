...
import { ARENA_HALF, PLAYER_HEIGHT, PLAYER_MAX_HEALTH, STARTING_WEAPON, WEAPON_ORDER, WEAPONS, type WeaponId } from './constants'
...
export class GameContext {
  ...
  weaponMag: Record<WeaponId, number> = Object.fromEntries(WEAPON_ORDER.map((id) => [id, 0])) as Record<WeaponId, number>
  weaponReserve: Record<WeaponId, number> = Object.fromEntries(WEAPON_ORDER.map((id) => [id, 0])) as Record<WeaponId, number>
