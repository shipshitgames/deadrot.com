...
/** Drops (blood/energy pickups + weapons): spawn, bob, collect. */
export class PickupsSystem {
  ...
  spawnPickup(kind: PickupKind, x: number, z: number) {
    ...
    if (isWeapon) {
      ...
      const sprite = new THREE.Sprite(mat);
      const scale = PICKUP_SPRITE_SCALES[kind];
  ...
  collectPickup(kind: PickupKind) {
    if (kind === "health") {
      this.ctx.health = Math.min(this.ctx.maxHealthValue, this.ctx.health + HEALTH_PICKUP_AMOUNT);
