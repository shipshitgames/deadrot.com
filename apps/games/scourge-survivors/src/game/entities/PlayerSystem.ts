...
export class PlayerSystem {
  ...
  private overlapsXZ(pos: THREE.Vector3, box: THREE.Box3, radius: number) {
    return pos.x > box.min.x - radius && pos.x < box.max.x + radius && pos.z > box.min.z - radius && pos.z < box.max.z + radius
  }
  ...
  damagePlayer(amount: number) {
    ...
    if (this.ctx.survivors) {
      ...
      const lowHealthArmor = this.ctx.statBastion > 0 && this.ctx.health / this.ctx.maxHealthValue <= 0.42 ? this.ctx.statBastion * 0.08 : 0
