...
import type { ArenaMap, ObstacleMat } from "../data/maps";
...
export class ArenaSystem {
  ...
  buildArena(map: ArenaMap) {
    ...
    const floorMat = new THREE.MeshStandardMaterial({
      map: this.makeRepeatingTexture(
        ARENA_TEXTURES.floor,
        ARENA_TEXTURE_REPEAT.floor[0],
        ARENA_TEXTURE_REPEAT.floor[1],
      ),
      color: t.floorTint,
      roughness: 0.9,
      metalness: 0.08,
    ...
    const crateMat = new THREE.MeshStandardMaterial({
      map: this.makeRepeatingTexture(
        ARENA_TEXTURES.block,
        ARENA_TEXTURE_REPEAT.block[0],
        ARENA_TEXTURE_REPEAT.block[1],
      ),
      color: t.wallTint,
      roughness: 0.72,
      metalness: 0.24,
    ...
    const pillarMat = new THREE.MeshStandardMaterial({
      map: this.makeRepeatingTexture(
        ARENA_TEXTURES.column,
        ARENA_TEXTURE_REPEAT.column[0],
        ARENA_TEXTURE_REPEAT.column[1],
      ),
      color: t.wallTint,
      roughness: 0.58,
      metalness: 0.32,
