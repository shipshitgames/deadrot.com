...
export class RemoteAvatar {
  ...
  constructor(info: RemoteAvatarInfo) {
    ...
    this.nameSprite = new THREE.Sprite(
      new THREE.SpriteMaterial({ map: this.nameTex, transparent: true, depthTest: false }),
    );
    ...
    this.healthFill = new THREE.Mesh(
      new THREE.PlaneGeometry(HB_WIDTH, 0.13),
    ...
    this.redrawName();
    document.fonts?.ready.then(() => this.redrawName()).catch(() => {});
  ...
  private applySprite(view: PlayerSpriteView = this.spriteView, flip = this.spriteFlip, elapsed = 0, moving = false) {
    ...
    const [baseW, baseH] = PLAYER_AVATAR_SCALES[this.avatar][view];
  ...
  private redrawName() {
    ...
    let fontSize = 18;
    ctx.font = `${fontSize}px ${NAMEPLATE_FONT_FAMILY}`;
    while (ctx.measureText(label).width > 286 && fontSize > 11) {
      fontSize -= 1;
      ctx.font = `${fontSize}px ${NAMEPLATE_FONT_FAMILY}`;
    }
