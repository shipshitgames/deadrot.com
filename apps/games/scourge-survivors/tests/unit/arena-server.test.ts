...
describe("PartyKit PvP arena preview authority", () => {
  it("catches up a joiner when sockets connected before either peer became visible", () => {
    const { arena, room, connect } = setup([
      { x: 0, z: 0 },
      { x: 2, z: 0 },
    ]);
    const a = connect("a", false);
    const b = connect("b", false);
    a.messages.length = 0;
    b.messages.length = 0;
    room.broadcasts = [];

    arena.onMessage(JSON.stringify({ t: "join", name: "Alpha", avatar: "ranger" }), a.connection);
    arena.onMessage(JSON.stringify({ t: "join", name: "Bravo", avatar: "heavy" }), b.connection);

    const bCatchup = b.messages.map((message) => JSON.parse(message)).find((message) => message.t === "join");
    expect(bCatchup?.player).toMatchObject({ id: "a", name: "Alpha", joined: true });
    expect(decoded(room.broadcasts, "join").at(-1)?.player).toMatchObject({ id: "b", name: "Bravo" });
  });
