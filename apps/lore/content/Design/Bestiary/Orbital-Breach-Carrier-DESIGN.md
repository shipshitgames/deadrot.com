---
type: design-lock
status: candidate-master
subject: Orbital-Breach-Carrier
subjectType: creature
canon: Bestiary/Orbital-Breach-Carrier
styleBible: Style-Bible
lockedDate:
---
# Orbital Breach Carrier Design Lock

Games adapt from this file; runtime frames do not define the Orbital Breach Carrier.

## Canon Links

- Canon page: [[Orbital-Breach-Carrier]]
- Style bible: [[Style-Bible]]
- Design tokens: [[DESIGN]]

## Visual Thesis

Living carrier-ship boss: infected wreckage grown around an exposed orbital breach
heart, with chitin over rusted hull ribs, spore vents, dangling armor, and damaged
engine tissue.

## Silhouette Lock

- Primary read: large arcade boss, not a normal fighter.
- Proportion: wide carrier mass with readable weakpoint center.
- Iconic shape signature: central breach heart framed by broken hull ribs and spore bays.
- Forbidden shape drift: clean spaceship, generic squid, smooth alien manta.

## Material And Color Lock

- Body materials: rusted ship metal, black chitin, raw engine tissue.
- Accent materials: spore vents, cracked hull ribs, dangling corrupted panels.
- Emissive color: toxic green at the breach heart and small vent nodes.
- Forbidden colors: clean sci-fi blue, purple space glow, smooth chrome.

## Locked Poses

Status: candidate master attached. Runtime preview below is placeholder only
until regenerated from this sheet.

![Orbital Breach Carrier master turnaround](/assets/lore/art-masters/scourge/orbital-breach-carrier/orbital-breach-carrier-master-turnaround.png)

![Orbital Breach Carrier runtime placeholder](/assets/lore/art-masters/scourge/orbital-breach-carrier/orbital-breach-carrier-runtime-placeholder.webp)

- Top/front boss sprite:
- Damage state 01:
- Damage state 02:
- Optional action/read pose: spore bays open around shielded breach heart.

## Art Master Attachments

- Candidate master: `/assets/lore/art-masters/scourge/orbital-breach-carrier/orbital-breach-carrier-master-turnaround.png`
- Generated source: `packages/assets/sources/generated/2026-06-11/lore/bestiary/scourge-foes/orbital-breach-carrier-master-turnaround.png`
- Runtime placeholder: `/assets/lore/art-masters/scourge/orbital-breach-carrier/orbital-breach-carrier-runtime-placeholder.webp`

## Game Adaptations

- Starblight: boss-scale carrier that spawns spore mines and [[Scourge-Fighter]] waves.

## Validation Checklist

- [ ] Boss silhouette approved.
- [x] Breach heart readable as weakpoint.
- [x] Source poses archived in `packages/assets/sources/generated`.
- [ ] Runtime frames regenerated from approved pose lock.
- [ ] No matte residue, stray pixels, or cropped limbs.
