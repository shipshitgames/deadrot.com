---
genre: MOBA (lane-based PvP)
repo: shipshitgames/pactfall
status: playable prototype
---

# Pactfall

**At a glance:** playable prototype / lane-based PvP design target · Pyre vs Wardens in sanctioned blood-arenas, with a neutral center Scourge that overruns both sides and can force a truce · current build tests the [[Bloodlane]] one-lane v1 format; full target is [[The-Pyre|Pyre]] and [[The-Wardens|Warden]] hero play · the human rivalry made playable where the Pact bends but must not break — the target PvP pillar feeding the [[War-for-the-Lanes]].

The playable prototype tests the arena pressure first: a Pyre champion, a Warden opponent,
lane minions, and a neutral Scourge problem in the center. The [[Bloodlane]] design doc locks
the first real v1 shape: one lane, base-seal win condition, center Scourge-jungle pressure,
and a forced-truce loop. The design target remains a PvP MOBA where [[The-Pyre]] and
[[The-Wardens]] settle their rivalry — fought over **lanes** (the
universe's signature ground), in sanctioned blood-arenas, **never as real war** (see
[[Premise]] §The Pact). Gory, fast, hellfire-lit.

## The Scourge twist

[[Scourge|The Scourge]] is the neutral threat in the center: ignore it and it overruns both
teams, and a big enough surge can force a momentary truce — because humans always unite
against it. A MOBA mechanic pulled straight from the canon.

## Canon role

The human rivalry made playable: a sanctioned arena where the Pact bends, but must not break.
The name points at the danger of the Pact falling, not just at lanes.

## Run loop

Uses the shared [[Survivors-Loop]]: escalating lane pressure, XP/scrap/blood drops, 1-of-3
upgrade drafts, build synergies, neutral Scourge surges, and boss/objective gates.

## Design target

[[Bloodlane]] is the current Pactfall v1 target: a one-lane arcade MOBA that proves the base
race, center Scourge-jungle, and forced-truce Pact mechanic before expanding into a full
roster or multiplayer contract.

## First Sprite Roster

- Pyre heroes: [[Pyre-Duelist]], [[Pyre-Cauterizer]].
- Warden heroes: [[Warden-Bastion]], [[Warden-Artillerist]].
- Neutral Scourge objective: [[Trucebreaker]].

## Status

Playable prototype — current build proves the readable arena/minion/neutral-Scourge loop, not
the full hero roster or PvP contract. Roster, balance, and faction coverage are provisional
until the PvP pillar is actually fun.
