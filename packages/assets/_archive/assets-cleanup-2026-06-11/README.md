...
This folder holds rejected or review-only files moved out of runtime asset paths
so `packages/assets` can stay CDN clean while still preserving useful asset
history. Curated source history for successful generations belongs in
`packages/assets/sources/generated/`.

Do not serve this folder as part of `/assets` or sync it to `cdn.deadrot.com`.
After review, delete files marked `delete`, keep notes that are still useful,
and regenerate art marked `regenerate` through the asset pipeline.
